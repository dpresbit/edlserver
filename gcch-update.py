...
datapath = Path(tempfile.gettempdir() + '/gcch_endpoints_clientid_latestversion.txt')
...
version = webApiGet('version', 'USGovGCCHigh', clientRequestId)

# UNCOMMENT if you want to force an update (you can also delete the tmp file in /tmp)
#latestVersion = '0'

if version['latest'] > latestVersion:
    print('New version of Office 365 DoD service instance endpoints detected')
    # write the new version number to the data file
    with open(datapath, 'w') as fout:
        fout.write(clientRequestId + '\n' + version['latest'])
    # invoke endpoints method to get the new data
    endpointSets = webApiGet('endpoints', 'USGovGCCHigh', clientRequestId)
...
#    print('\n'.join(sorted(set([ip for (category, ip, tcpPorts, udpPorts) in flatIps]))))
    ...
    with open('/home/fedadmin/edlserver/gcchedl.txt', 'w') as f:
            f.write("%s\n" % MyURLList)

    # TODO send mail (e.g. with smtplib/email modules) with new endpoints data
else:
    print('Office 365 USGovDoD service instance endpoints are up-to-date')
